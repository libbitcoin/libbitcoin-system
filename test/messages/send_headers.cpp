...
{
    const messages::send_headers expected{};
    const auto data = expected.to_data(messages::version::level::maximum);
    const auto result = messages::send_headers::factory(
...
BOOST_AUTO_TEST_CASE(from_data_reader_version_prior_bip130_failure)
{
    data_chunk data{};
    read::bytes::copy source(data);
    messages::send_headers instance{};
    const auto result = instance.from_data(messages::version::level::bip130 - 1, source);
...
BOOST_AUTO_TEST_CASE(from_data_reader_version_at_least_bip130_success)
{
    ...
    read::bytes::copy source(data);
    messages::send_headers instance{};
    const auto result = instance.from_data(messages::version::level::bip130, source);
