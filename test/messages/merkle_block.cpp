...
{
    const messages::merkle_block instance;
...
BOOST_AUTO_TEST_CASE(merkle_block__constructor_2__always__equals_params)
{
    ...
    {
        base16_hash("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaffffffffffffffffffffffffffffffff"),
        base16_hash("bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee"),
        base16_hash("ccccccccccccccccccccccccccccccccdddddddddddddddddddddddddddddddd"),
    };
    const data_chunk flags{ 0xae, 0x56, 0x0f };

    messages::merkle_block instance(header, count, hashes, flags);
    BOOST_REQUIRE(instance.is_valid());
...
BOOST_AUTO_TEST_CASE(merkle_block__constructor_4__always__equals_params)
{
    ...
    messages::merkle_block instance(expected);
    BOOST_REQUIRE(instance.is_valid());
...
BOOST_AUTO_TEST_CASE(merkle_block__constructor_5__always__equals_params)
{
    ...
    messages::merkle_block expected(header, count, hashes, flags);
    messages::merkle_block instance(std::move(expected));
    BOOST_REQUIRE(instance.is_valid());
    BOOST_REQUIRE(header == instance.header());
    BOOST_REQUIRE_EQUAL(instance.total_transactions(), count);
    BOOST_REQUIRE(hashes == instance.hashes());
...
BOOST_AUTO_TEST_CASE(from_data_insufficient_data_fails)
{
    ...
    messages::merkle_block instance;

    BOOST_REQUIRE(!instance.from_data(messages::version::level::maximum, data));
    BOOST_REQUIRE(!instance.is_valid());
}
...
BOOST_AUTO_TEST_CASE(from_data_insufficient_version_fails)
{
    ...
    BOOST_REQUIRE(!instance.from_data(
        messages::merkle_block::version_minimum - 1, data));
    BOOST_REQUIRE(!instance.is_valid());
}
...
BOOST_AUTO_TEST_CASE(roundtrip_to_data_factory_chunk)
{
    ...
    const auto data = expected.to_data(messages::version::level::maximum);
    const auto result = messages::merkle_block::factory(messages::version::level::maximum, data);
...
BOOST_AUTO_TEST_CASE(roundtrip_to_data_factory_stream)
{
    ...
    const auto result = messages::merkle_block::factory(messages::version::level::maximum, istream);
...
BOOST_AUTO_TEST_CASE(roundtrip_to_data_factory_reader)
{
    ...
    const auto result = messages::merkle_block::factory(messages::version::level::maximum, source);
