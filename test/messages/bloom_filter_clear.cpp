...
{
    static const bloom_filter_clear expected{};
    const auto raw = expected.to_data(version::level::maximum);
    bloom_filter_clear instance{};

    BOOST_REQUIRE(!instance.from_data(bloom_filter_clear::version_minimum - 1, raw));
...
BOOST_AUTO_TEST_CASE(roundtrip_to_data_factory_chunk)
{
    ...
    const auto result = bloom_filter_clear::factory(version::level::maximum, data);
...
BOOST_AUTO_TEST_CASE(roundtrip_to_data_factory_stream)
{
    ...
    const auto result = bloom_filter_clear::factory(version::level::maximum, istream);
...
BOOST_AUTO_TEST_CASE(roundtrip_to_data_factory_reader)
{
    ...
    const auto result = bloom_filter_clear::factory(version::level::maximum, source);
