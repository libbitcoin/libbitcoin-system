...
script test_prevout(const script_test& test)
{
    ...
    const script output_script(test.output);
    return output_script.is_valid() ? output_script : script{};
}

transaction test_tx(const script_test& test)
{
    // Parse input script from string.
    const script input_script(test.input);
    if (!input_script.is_valid())
        return {};
...
BOOST_AUTO_TEST_CASE(script__from_string__two_of_three_multisig__success)
{
    ...
    BOOST_REQUIRE_EQUAL(ops[2].to_string(forks::no_rules), "[02440e0304bf8d32b2012994393c6a477acf238dd6adb4c3cef5bfa72f30c9861c]");
    BOOST_REQUIRE_EQUAL(ops[3].to_string(forks::no_rules), "[03624505c6cc3967352cce480d8550490dd68519cd019066a4c302fdfb7d1c9934]");
...
BOOST_AUTO_TEST_CASE(script__bip16__valid)
{
    for (const auto& test: valid_bip16_scripts)
    {
        ...
        const auto prevout = test_prevout(test);
        BOOST_REQUIRE_MESSAGE(tx.is_valid(), name);

        // These are valid prior to and after BIP16 activation.
        BOOST_CHECK_MESSAGE(script::verify(tx, 0, forks::no_rules, prevout, 0) == error::success, name);
        BOOST_CHECK_MESSAGE(script::verify(tx, 0, forks::bip16_rule, prevout, 0) == error::success, name);
        BOOST_CHECK_MESSAGE(script::verify(tx, 0, forks::all_rules, prevout, 0) == error::success, name);
    }
}
...
BOOST_AUTO_TEST_CASE(script__bip65__valid)
{
    for (const auto& test: valid_bip65_scripts)
    {
        ...
        BOOST_CHECK_MESSAGE(script::verify(tx, 0, forks::no_rules, prevout, 0) == error::success, name);
        BOOST_CHECK_MESSAGE(script::verify(tx, 0, forks::bip65_rule, prevout, 0) == error::success, name);
        BOOST_CHECK_MESSAGE(script::verify(tx, 0, forks::all_rules & ~forks::bip112_rule, prevout, 0) == error::success, name);
    }
}
...
BOOST_AUTO_TEST_CASE(script__multisig__valid)
{
    for (const auto& test: valid_multisig_scripts)
    {
        ...
        BOOST_CHECK_MESSAGE(script::verify(tx, 0, forks::no_rules, prevout, 0) == error::success, name);
        BOOST_CHECK_MESSAGE(script::verify(tx, 0, forks::bip66_rule, prevout, 0) == error::success, name);
        BOOST_CHECK_MESSAGE(script::verify(tx, 0, forks::all_rules, prevout, 0) == error::success, name);
    }
}
...
BOOST_AUTO_TEST_CASE(script__checksig__single__uses_one_hash)
{
    ...
    BOOST_REQUIRE(script::check_signature(signature, pubkey, subscript, parent_tx, index, value, coverage::hash_single, script_version::unversioned, bip143));
}
...
BOOST_AUTO_TEST_CASE(script__create_endorsement__single_input_single_output__expected)
{
    ...
    const auto bip143 = false;
    endorsement out;
    BOOST_REQUIRE(script::create_endorsement(out, secret, prevout_script, test_tx, index, value, coverage::hash_all, script_version::unversioned, bip143));
...
BOOST_AUTO_TEST_CASE(script__verify__testnet_block_23428_multisig_tx__success)
{
    ...
    const transaction tx(decoded_tx, true);
    BOOST_REQUIRE(tx.is_valid());
    BOOST_REQUIRE_GT(tx.inputs()->size(), index);

    const auto decoded_script = base16_chunk("a9144aba54e2541475f91659ccdbb13ce0b490778c7f87");
    const script prevout(decoded_script, false);
    BOOST_REQUIRE(prevout.is_valid());

    ////std::cout << prevout().to_string(forks) << std::endl;
    ////std::cout << input.script().to_string(forks) << std::endl;
    ////std::cout << input.witness().to_string() << std::endl;

    BOOST_REQUIRE_EQUAL(script::verify(tx, index, forks, prevout, 0), error::success);
}
...
BOOST_AUTO_TEST_CASE(script__verify__block_290329_tx__success)
{
    ...
    const auto decoded_script = base16_chunk("a914d8dacdadb7462ae15cd906f1878706d0da8660e687");
    const script prevout(decoded_script, false);
    BOOST_REQUIRE(prevout.is_valid());
    BOOST_REQUIRE_EQUAL(script::verify(tx, index, forks, prevout, value), error::success);
}
...
BOOST_AUTO_TEST_CASE(script__verify__block_438513_tx__success)
{
    ...
    static const auto value = 0u;
    const auto decoded_script = base16_chunk("a914faa558780a5767f9e3be14992a578fc1cbcf483087");
    const script prevout(decoded_script, false);
    BOOST_REQUIRE(prevout.is_valid());
    BOOST_REQUIRE_EQUAL(script::verify(tx, index, forks, prevout, value), error::success);
}
...
BOOST_AUTO_TEST_CASE(script__verify__block_481824_tx__success)
{
    ...
    const auto decoded_script = base16_chunk("a9142928f43af18d2d60e8a843540d8086b30534133987");
    const script prevout(decoded_script, false);
    BOOST_REQUIRE(prevout.is_valid());
    BOOST_REQUIRE_EQUAL(script::verify(tx, index, forks, prevout, value), error::success);
}
...
BOOST_AUTO_TEST_CASE(script__verify__testnet_block_892321_tx_missing_witness__invalid_witness)
{
    ...
    const auto decoded_script = base16_chunk("0020925fe0a6cde95bdc7a21b08925c246cae17005f8a013efffdb5e5cb7b7f8d0c2");
    const script prevout(decoded_script, false);
    BOOST_REQUIRE(prevout.is_valid());

    // This is correctly invalid.
    BOOST_REQUIRE_EQUAL(script::verify(tx, index, forks, prevout, value), error::invalid_witness);
}
...
BOOST_AUTO_TEST_CASE(script__verify__bip143_native_p2wpkh_tx__success)
{
    ...
    const auto value0 = 625000000;
    const auto decoded_script0 = base16_chunk("2103c9f4836b9a4f77fc0d81f7bcb01b7f1b35916864b9476c241ce9fc198bd25432ac");
    const script prevout0(decoded_script0, false);
    BOOST_REQUIRE(prevout0.is_valid());

    const auto value1 = 600000000;
    const auto decoded_script1 = base16_chunk("00141d0f172a0ecb48aee1be1f2687d2963ae33f71a1");
    const script prevout1(decoded_script1, false);
    BOOST_REQUIRE(prevout1.is_valid());

    // ordinary P2PK (no rules required).
    BOOST_REQUIRE_EQUAL(script::verify(tx, 0, forks::no_rules, prevout0, value0), error::success);

    // P2WPKH witness program.
    BOOST_REQUIRE_EQUAL(script::verify(tx, 1, forks::bip141_rule | forks::bip143_rule, prevout1, value1), error::success);

    // Other scenarios:

    // extra rules (okay).
    BOOST_REQUIRE_EQUAL(script::verify(tx, 0, forks::bip141_rule | forks::bip143_rule, prevout0, value0), error::success);

    // missing bip143 (invalid sighash).
    BOOST_REQUIRE_EQUAL(script::verify(tx, 1, forks::bip141_rule, prevout1, value1), error::stack_false);

    // missing bip141 (witness not allowed).
    BOOST_REQUIRE_EQUAL(script::verify(tx, 1, forks::bip143_rule, prevout1, value1), error::unexpected_witness);

    // missing bip141 (witness not allowed).
    BOOST_REQUIRE_EQUAL(script::verify(tx, 1, forks::no_rules, prevout1, value1), error::unexpected_witness);
}
...
BOOST_AUTO_TEST_CASE(script__verify__bip143_p2sh_p2wpkh_tx__success)
{
    ...
    const auto value = 1000000000;
    const auto decoded_script = base16_chunk("a9144733f37cf4db86fbc2efed2500b4f4e49f31202387");
    const script prevout(decoded_script, false);
    BOOST_REQUIRE(prevout.is_valid());

    // P2SH-P2WPKH witness program.
    BOOST_REQUIRE_EQUAL(script::verify(tx, 0, forks::bip16_rule | forks::bip141_rule | forks::bip143_rule, prevout, value), error::success);

    // Other scenarios:

    // missing bip16 (required for p2sh, embedded witness not consumed).
    BOOST_REQUIRE_EQUAL(script::verify(tx, 0, forks::bip141_rule | forks::bip143_rule, prevout, value), error::unexpected_witness);

    // missing bip141 (witness not allowed).
    BOOST_REQUIRE_EQUAL(script::verify(tx, 0, forks::bip16_rule | forks::bip143_rule, prevout, value), error::unexpected_witness);

    // missing bip143 (invalid sighash).
    BOOST_REQUIRE_EQUAL(script::verify(tx, 0, forks::bip16_rule | forks::bip141_rule, prevout, value), error::stack_false);
}
...
BOOST_AUTO_TEST_CASE(script__verify__bip143_native_p2wsh_1_tx__success)
{
    ...
    const auto value0 = 156250000;
    const auto decoded_script0 = base16_chunk("21036d5c20fa14fb2f635474c1dc4ef5909d4568e5569b79fc94d3448486e14685f8ac");

    const script prevout0(decoded_script0, false);
    BOOST_REQUIRE(prevout0.is_valid());

    const auto value1 = 4900000000;
    const auto decoded_script1 = base16_chunk("00205d1b56b63d714eebe542309525f484b7e9d6f686b3781b6f61ef925d66d6f6a0");
    const script prevout1(decoded_script1, false);
    BOOST_REQUIRE(prevout1.is_valid());
    ...
    // extra rules (okay).
    BOOST_REQUIRE_EQUAL(script::verify(tx, 0, forks::bip16_rule | forks::bip141_rule | forks::bip143_rule, prevout0, value0), error::success);

    // missing bip143 (code-separator treatment).
    BOOST_REQUIRE_EQUAL(script::verify(tx, 1, forks::bip141_rule, prevout1, value1), error::op_check_sig_verify5);
...
BOOST_AUTO_TEST_CASE(script__verify__bip143_native_p2wsh_2_tx__success)
{
    ...
    const auto value0 = 16777215;
    const auto decoded_script0 = base16_chunk("0020ba468eea561b26301e4cf69fa34bde4ad60c81e70f059f045ca9a79931004a4d");
    const script prevout0(decoded_script0, false);
    BOOST_REQUIRE(prevout0.is_valid());
    BOOST_REQUIRE(!(*tx.inputs())[0]->witness().stack().empty());
    BOOST_REQUIRE(!(*tx.inputs())[1]->witness().stack().empty());

    const auto value1 = 16777215;
    const auto decoded_script1 = base16_chunk("0020d9bbfbe56af7c4b7f960a70d7ea107156913d9e5a26b0a71429df5e097ca6537");
    const script prevout1(decoded_script1, false);
    BOOST_REQUIRE(prevout1.is_valid());
    ...
    // extra rules (okay).
    BOOST_REQUIRE_EQUAL(script::verify(tx, 1, forks::bip16_rule | forks::bip141_rule | forks::bip143_rule, prevout1, value1), error::success);

    // missing bip143 (code-separator treatment).
    BOOST_REQUIRE_EQUAL(script::verify(tx, 0, forks::bip141_rule, prevout0, value0), error::stack_false);

    // missing bip143 (code-separator treatment).
    BOOST_REQUIRE_EQUAL(script::verify(tx, 1, forks::bip141_rule, prevout1, value1), error::stack_false);

    // missing bip141 (witness not allowed).
    BOOST_REQUIRE_EQUAL(script::verify(tx, 0, forks::bip143_rule, prevout0, value0), error::unexpected_witness);
...
BOOST_AUTO_TEST_CASE(script__verify__bip143_native_p2wsh_3_tx__success)
{
    ...
    const auto value0 = 16777215;
    const auto decoded_script0 = base16_chunk("0020d9bbfbe56af7c4b7f960a70d7ea107156913d9e5a26b0a71429df5e097ca6537");
    const script prevout0(decoded_script0, false);
    BOOST_REQUIRE(prevout0.is_valid());

    const auto value1 = 16777215;
    const auto decoded_script1 = base16_chunk("0020ba468eea561b26301e4cf69fa34bde4ad60c81e70f059f045ca9a79931004a4d");
    const script prevout1(decoded_script1, false);
    BOOST_REQUIRE(prevout1.is_valid());
...
BOOST_AUTO_TEST_CASE(script__verify__bip143_p2sh_p2wsh_tx__success)
{
    ...
    const auto value0 = 987654321;
    const auto decoded_script0 = base16_chunk("a9149993a429037b5d912407a71c252019287b8d27a587");
    const script prevout0(decoded_script0, false);
    BOOST_REQUIRE(prevout0.is_valid());
    ...
    // missing bip16 (required for p2sh, embedded witness not consumed).
    BOOST_REQUIRE_EQUAL(script::verify(tx, 0, forks::bip141_rule | forks::bip143_rule, prevout0, value0), error::unexpected_witness);

    // missing bip141 (witness not allowed).
    BOOST_REQUIRE_EQUAL(script::verify(tx, 0, forks::bip16_rule | forks::bip143_rule, prevout0, value0), error::unexpected_witness);

    // missing bip143 (invalid sighash).
    BOOST_REQUIRE_EQUAL(script::verify(tx, 0, forks::bip16_rule | forks::bip141_rule, prevout0, value0), error::stack_false);
}
...
BOOST_AUTO_TEST_CASE(script__verify__bip143_no_find_and_delete_tx__success)
{
    ...
    const auto value0 = 200000;
    const auto decoded_script0 = base16_chunk("00209e1be07558ea5cc8e02ed1d80c0911048afad949affa36d5c3951e3159dbea19");
    const script prevout0(decoded_script0, false);
    BOOST_REQUIRE(prevout0.is_valid());

    // P2WSH witness program.
    BOOST_REQUIRE_EQUAL(script::verify(tx, 0, forks::bip141_rule | forks::bip143_rule | forks::bip147_rule, prevout0, value0), error::success);
    ...
    BOOST_REQUIRE_EQUAL(script::verify(tx, 0, forks::bip16_rule | forks::bip141_rule, prevout0, value0), error::op_check_sig_verify5);
}
