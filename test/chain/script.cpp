...
BOOST_AUTO_TEST_CASE(script__bip16__valid)
{
    for (const auto& test: valid_bip16_scripts)
    {
        ...
        BOOST_CHECK_MESSAGE(tx.connect({ forks::bip16_rule }, 0) == error::script_success, name);
...
BOOST_AUTO_TEST_CASE(script__verify__testnet_block_23428_multisig_tx__success)
{
    ...
    (*tx.inputs())[index]->prevout.reset(new prevout{ value, { base16_chunk("a9144aba54e2541475f91659ccdbb13ce0b490778c7f87"), false } });
...
BOOST_AUTO_TEST_CASE(script__verify__block_290329_tx__success)
{
    ...
    (*tx.inputs())[index]->prevout.reset(new prevout{ value, { base16_chunk("a914d8dacdadb7462ae15cd906f1878706d0da8660e687"), false } });
    BOOST_REQUIRE_EQUAL(tx.connect({ forks }, index), error::script_success);
}
...
BOOST_AUTO_TEST_CASE(script__verify__block_438513_tx__success)
{
    ...
    static const auto value = 0u;
    (*tx.inputs())[index]->prevout.reset(new prevout{ value, { base16_chunk("a914faa558780a5767f9e3be14992a578fc1cbcf483087"), false } });
    BOOST_REQUIRE_EQUAL(tx.connect({ forks }, index), error::script_success);
}
...
BOOST_AUTO_TEST_CASE(script__verify__block_481824_tx__success)
{
    ...
    (*tx.inputs())[index]->prevout.reset(new prevout{ value, { base16_chunk("a9142928f43af18d2d60e8a843540d8086b30534133987"), false } });
    BOOST_REQUIRE_EQUAL(tx.connect({ forks }, index), error::script_success);
}
...
BOOST_AUTO_TEST_CASE(script__verify__testnet_block_892321_tx_missing_witness__invalid_witness)
{
    ...
    (*tx.inputs())[index]->prevout.reset(new prevout{ value, { base16_chunk("0020925fe0a6cde95bdc7a21b08925c246cae17005f8a013efffdb5e5cb7b7f8d0c2"), false } });
...
BOOST_AUTO_TEST_CASE(script__verify__bip143_native_p2wpkh_tx__success)
{
    ...
    (*tx.inputs())[0]->prevout.reset(new prevout{ value0, { base16_chunk("2103c9f4836b9a4f77fc0d81f7bcb01b7f1b35916864b9476c241ce9fc198bd25432ac"), false } });

    const auto value1 = 600000000u;
    (*tx.inputs())[1]->prevout.reset(new prevout{ value1, { base16_chunk("00141d0f172a0ecb48aee1be1f2687d2963ae33f71a1"), false } });
...
BOOST_AUTO_TEST_CASE(script__verify__bip143_p2sh_p2wpkh_tx__success)
{
    ...
    (*tx.inputs())[0]->prevout.reset(new prevout{ value, { base16_chunk("a9144733f37cf4db86fbc2efed2500b4f4e49f31202387"), false } });
    ...
    BOOST_REQUIRE_EQUAL(tx.connect({ forks::bip141_rule | forks::bip143_rule }, 0), error::unexpected_witness);

    // missing bip141 (witness not allowed).
    BOOST_REQUIRE_EQUAL(tx.connect({ forks::bip16_rule | forks::bip143_rule }, 0), error::unexpected_witness);

    // missing bip143 (invalid sighash).
    BOOST_REQUIRE_EQUAL(tx.connect({ forks::bip16_rule | forks::bip141_rule }, 0), error::stack_false);
}
...
BOOST_AUTO_TEST_CASE(script__verify__bip143_native_p2wsh_1_tx__success)
{
    ...
    (*tx.inputs())[0]->prevout.reset(new prevout{ value0, { base16_chunk("21036d5c20fa14fb2f635474c1dc4ef5909d4568e5569b79fc94d3448486e14685f8ac"), false } });

    const auto value1 = 4900000000u;
    (*tx.inputs())[1]->prevout.reset(new prevout{ value1, { base16_chunk("00205d1b56b63d714eebe542309525f484b7e9d6f686b3781b6f61ef925d66d6f6a0"), false } });
...
BOOST_AUTO_TEST_CASE(script__verify__bip143_native_p2wsh_2_tx__success)
{
    ...
    (*tx.inputs())[0]->prevout.reset(new prevout{ value0, { base16_chunk("0020ba468eea561b26301e4cf69fa34bde4ad60c81e70f059f045ca9a79931004a4d"), false } });

    const auto value1 = 16777215u;
    (*tx.inputs())[1]->prevout.reset(new prevout{ value1, { base16_chunk("0020d9bbfbe56af7c4b7f960a70d7ea107156913d9e5a26b0a71429df5e097ca6537"), false } });
    ...
    // all inputs
    BOOST_REQUIRE_EQUAL(tx.connect({ forks::bip141_rule | forks::bip143_rule }), error::transaction_success);
    ...
    // extra rules (okay).
    BOOST_REQUIRE_EQUAL(tx.connect({ forks::bip16_rule | forks::bip141_rule | forks::bip143_rule }, 1), error::script_success);

    // missing bip143 (code-separator treatment).
    BOOST_REQUIRE_EQUAL(tx.connect({ forks::bip141_rule }, 0), error::stack_false);

    // missing bip143 (code-separator treatment).
    BOOST_REQUIRE_EQUAL(tx.connect({ forks::bip141_rule }, 1), error::stack_false);

    // missing bip141 (witness not allowed).
    BOOST_REQUIRE_EQUAL(tx.connect({ forks::bip143_rule }, 0), error::unexpected_witness);
...
BOOST_AUTO_TEST_CASE(script__verify__bip143_p2sh_p2wsh_tx__success)
{
    ...
    (*tx.inputs())[0]->prevout.reset(new prevout{ value0, { base16_chunk("a9149993a429037b5d912407a71c252019287b8d27a587"), false } });
...
BOOST_AUTO_TEST_CASE(script__verify__bip143_no_find_and_delete_tx__success)
{
    ...
    (*tx.inputs())[0]->prevout.reset(new prevout{ value0, { base16_chunk("00209e1be07558ea5cc8e02ed1d80c0911048afad949affa36d5c3951e3159dbea19"), false } });

    // P2WSH witness program.
    BOOST_REQUIRE_EQUAL(tx.connect({ forks::bip141_rule | forks::bip143_rule | forks::bip147_rule }, 0), error::script_success);
