...
BOOST_AUTO_TEST_CASE(script__create_endorsement__single_input_single_output__expected)
{
    ...
    const auto value = 0u;
    const auto bip143 = false;
    BOOST_REQUIRE(script::create_endorsement(out, secret, prevout_script, new_tx, index, value, sighash_algorithm::hash_all, script_version::unversioned, bip143));
...
BOOST_AUTO_TEST_CASE(script__generate_signature_hash__all__expected)
{
    ...
    const auto value = 0u;
    const auto bip143 = false;
    const auto sighash = script::generate_signature_hash(new_tx, index, prevout_script, value, sighash_algorithm::hash_all, script_version::unversioned, bip143);
...
BOOST_AUTO_TEST_CASE(script__verify__bip143_native_p2wsh_1_tx__valid)
{
    ...
    result1 = script::verify(tx, 1, rule_fork::bip141_rule);
    BOOST_REQUIRE_EQUAL(result1.value(), error::op_check_sig_verify5);
...
BOOST_AUTO_TEST_CASE(script__verify__bip143_no_find_and_delete_tx__valid)
{
    ...
    result0 = script::verify(tx, 0, rule_fork::bip16_rule | rule_fork::bip141_rule);
    BOOST_REQUIRE_EQUAL(result0.value(), error::op_check_sig_verify5);
}
