...
BOOST_AUTO_TEST_CASE(intrinsics__intel_128__andnot__expected)
{
    if constexpr (have_128)
    {
        const auto xword = f::andnot(f::set<xint128_t>(0xf0f0_u64, 0xffff_u64), f::set<xint128_t>(0xffff_u64, 0x0f0f_u64));
        BOOST_CHECK_EQUAL((f::get<uint64_t, 0>(xword)), 0x0f0f_u64);
        BOOST_CHECK_EQUAL((f::get<uint64_t, 1>(xword)), 0x0000_u64);
    }
}

BOOST_AUTO_TEST_CASE(intrinsics__intel_128__eq64__expected)
{
    if constexpr (have_128)
    {
        const auto xword = f::eq<64>(f::set<xint128_t>(1_u64, 2_u64), f::set<xint128_t>(1_u64, 3_u64));
        BOOST_CHECK_EQUAL((f::get<uint64_t, 0>(xword)), max_uint64);
        BOOST_CHECK_EQUAL((f::get<uint64_t, 1>(xword)), 0_u64);
    }
}

BOOST_AUTO_TEST_CASE(intrinsics__intel_128__eq32__expected)
{
    if constexpr (have_128)
    {
        const auto xword = f::eq<32>(f::set<xint128_t>(1_u32, 2_u32, 3_u32, 4_u32), f::set<xint128_t>(1_u32, 0_u32, 3_u32, 0_u32));
        BOOST_CHECK_EQUAL((f::get<uint32_t, 0>(xword)), max_uint32);
        BOOST_CHECK_EQUAL((f::get<uint32_t, 1>(xword)), 0_u32);
        BOOST_CHECK_EQUAL((f::get<uint32_t, 2>(xword)), max_uint32);
        BOOST_CHECK_EQUAL((f::get<uint32_t, 3>(xword)), 0_u32);
    }
}

BOOST_AUTO_TEST_CASE(intrinsics__intel_128__select__expected)
{
    if constexpr (have_128)
    {
        const auto mask = f::set<xint128_t>(max_uint64, 0_u64);
        const auto xword = f::select(mask, f::set<xint128_t>(10_u64, 20_u64), f::set<xint128_t>(30_u64, 40_u64));
        BOOST_CHECK_EQUAL((f::get<uint64_t, 0>(xword)), 10_u64);
        BOOST_CHECK_EQUAL((f::get<uint64_t, 1>(xword)), 40_u64);
    }
}

BOOST_AUTO_TEST_CASE(intrinsics__intel_128__any__expected)
{
    if constexpr (have_128)
    {
        BOOST_CHECK(!f::any(f::set<xint128_t>(0_u64, 0_u64)));
        BOOST_CHECK(f::any(f::set<xint128_t>(0_u64, 1_u64)));
        BOOST_CHECK(f::any(f::set<xint128_t>(0x8000000000000000_u64, 0_u64)));
    }
}
...
BOOST_AUTO_TEST_CASE(intrinsics__intel_128__gather__expected)
{
    if constexpr (have_128)
    {
        constexpr std_array<uint64_t, 8> table{ 100, 101, 102, 103, 104, 105, 106, 107 };
        const auto xword = f::gather(table.data(), f::set<xint128_t>(3_u64, 0_u64));
