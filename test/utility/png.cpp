...
BOOST_AUTO_TEST_CASE(png__size_one__success)
{
    ...
    const unsigned char raw_input_data[] = {
    ...
    data_chunk input_data = to_chunk(raw_input_data);
    ...
    const auto& expected_data_len = 285;

    const uint32_t size = 1;
    const auto ret = write_png(input_data, size, ostream);

    BOOST_REQUIRE_EQUAL(out.size(), expected_data_len);
    BOOST_REQUIRE_EQUAL(std::memcmp(
        out.data(), expected_data, expected_data_len), 0);
}
