...
BOOST_AUTO_TEST_CASE(hash__scrypt__rfc7914_salsa_8__expected)
{
    // W/R/P unused.
    ...
    test::salsa_8(data);
    BOOST_REQUIRE_EQUAL(data, base16_array("a41f859c6608cc993b81cacb020cef05044b2181a2fd337dfd7b1c6396682f29b4393168e3c9e6bcfe6bc5b7a06d96bae424cc102c91745c24ad673dc7618f81"));
}

BOOST_AUTO_TEST_CASE(hash__scrypt__rfc7914_block_mix__expected)
{
    // W/P unused.
    using test = scrypt_accessor<16, 1, 1, true>;
    ...
    BOOST_REQUIRE_EQUAL(data, base16_array(""));
}

BOOST_AUTO_TEST_CASE(hash__scrypt__rfc7914_romix__expected)
{
    // P unused.
    using test = scrypt_accessor<16, 1, 1, true>;
    auto data = base16_array("");
...
BOOST_AUTO_TEST_CASE(hash__scrypt__rfc7914_hash_2__expected)
{
    ...
    constexpr auto size = size_of<decltype(expected)>();
    const auto hash = scrypt<1024, 8, 16, true>::hash<size>("password", "NaCl");
    BOOST_REQUIRE_EQUAL(hash, expected);
}
...
BOOST_AUTO_TEST_CASE(hash__scrypt__rfc7914_hash_3__expected)
{
    ...
    constexpr auto size = size_of<decltype(expected)>();
    const auto hash = scrypt<16384, 8, 1, true>::hash<size>("pleaseletmein", "SodiumChloride");
    BOOST_REQUIRE_EQUAL(hash, expected);
}
...
BOOST_AUTO_TEST_CASE(hash__scrypt__rfc7914_hash_4__expected)
{
    ...
    constexpr auto size = size_of<decltype(expected)>();
    const auto hash = scrypt<1048576, 8, 1, true>::hash<size>("pleaseletmein", "SodiumChloride");
    BOOST_REQUIRE_EQUAL(hash, expected);
}
#endif

BOOST_AUTO_TEST_CASE(hash__scrypt__rfc7914_pbkdf2_hmac_sha256_1__expected)
{
    // c=1, dkLen=64
    data_chunk chunk(64);
    constexpr auto it = 1u;
    const auto pass = to_chunk("passwd");
    const auto salt = to_chunk("salt");
    pbkd::sha256::hash(pass.data(), pass.size(), salt.data(), salt.size(), it, chunk.data(), chunk.size());
    BOOST_REQUIRE_EQUAL(chunk, base16_chunk("55ac046e56e3089fec1691c22544b605f94185216dde0465e68b9d57c20dacbc49ca9cccf179b645991664b39d77ef317c71b845b1e30bd509112041d3a19783"));
}

BOOST_AUTO_TEST_CASE(hash__scrypt__rfc7914_pbkdf2_hmac_sha256_2__expected)
{
    // c=80000, dkLen=64
    const auto expected = base16_chunk("4ddcd8f60b98be21830cee5ef22701f9641a4418d04c0414aeff08876b34ab56a1d425a1225833549adb841b51c9b3176a272bdebba1d078478f62b397f33c8d");
    data_chunk chunk(expected.size());
    constexpr auto it = 80000u;
    const auto pass = to_chunk("Password");
    const auto salt = to_chunk("NaCl");
    pbkd::sha256::hash(pass.data(), pass.size(), salt.data(), salt.size(), it, chunk.data(), chunk.size());
