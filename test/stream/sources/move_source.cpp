...
BOOST_AUTO_TEST_CASE(move_source__input_sequence__empty__empty)
{
    data_chunk sink;
...
BOOST_AUTO_TEST_CASE(move_source__input_sequence__not_empty__expected)
{
...
{
    move_source<data_slab> instance(data_chunk{ 0x42 });
...
////    move_source<data_slab> instance(data_chunk(add1<int>(max_uint8), 0x42));
...
BOOST_AUTO_TEST_CASE(move_source__read__multiple__correct_tracking)
{
    ...
    move_source<data_slab> instance(data_chunk{ 0x01, 0x02, 0x03, 0x04, 0x05, 0x06 });
