...
BOOST_AUTO_TEST_CASE(hd_keys_vector1)
{
    ...
    data_chunk seed{0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
...
BOOST_AUTO_TEST_CASE(hd_keys_vector2)
{
    ...
    data_chunk seed{0xff, 0xfc, 0xf9, 0xf6, 0xf3, 0xf0, 0xed, 0xea,
...
BOOST_AUTO_TEST_CASE(hd_keys_serialize)
{
    ...
    BOOST_REQUIRE(public_key.set_encoded(public_string));
    BOOST_REQUIRE(public_key.encoded() == public_string);

    // Invalid public keys:
    BOOST_REQUIRE(!public_key.set_encoded(private_string));
    BOOST_REQUIRE(!public_key.set_encoded(public_string + ' '));

    // Simple private key round trip:
    hd_private_key private_key;
    BOOST_REQUIRE(private_key.set_encoded(private_string));
    BOOST_REQUIRE(private_key.encoded() == private_string);

    // Invalid private keys:
    BOOST_REQUIRE(!private_key.set_encoded(public_string));
    BOOST_REQUIRE(!private_key.set_encoded(private_string + ' '));

    // Casting away private part:
    hd_public_key& public_part = private_key;
    BOOST_REQUIRE(public_part.encoded() == public_string);
}
...
BOOST_AUTO_TEST_CASE(hd_keys_serialize_derive)
{
    ...
    hd_private_key private_key;
    BOOST_REQUIRE(private_key.set_encoded(private_string));
    BOOST_REQUIRE(private_key.generate_private_key(0).encoded() ==
        "xprv9vHkqa6EV4sPZHYqZznhT2NPtPCjKuDKGY38FBWLvgaDx45zo9WQ"
        "RUT3dKYnjwih2yJD9mkrocEZXo1ex8G81dwSM1fwqWpWkeS3v86pgKt");
}
