...
{
    init.init();
    return secp256k1_ec_pubkey_verify(public_key.data(), public_key.size()) != 0;
}

BC_API bool verify_private_key(const ec_secret& private_key)
{
    init.init();
    return secp256k1_ec_seckey_verify(private_key.data()) != 0;
}
...
BC_API data_chunk sign(ec_secret secret, hash_digest hash, ec_secret nonce)
{
    ...
    bool valid = secp256k1_ecdsa_sign(hash.data(), hash.size(),
        signature.data(), &out_size, secret.data(), nonce.data()) != 0;
    if (!valid)
        return data_chunk();
...
{
    init.init();
    return secp256k1_ec_pubkey_tweak_add(a.data(), a.size(), b.data()) != 0;
}

BC_API bool ec_multiply(ec_point& a, const ec_secret& b)
{
    init.init();
    return secp256k1_ec_pubkey_tweak_mul(a.data(), a.size(), b.data()) != 0;
}

BC_API bool ec_add(ec_secret& a, const ec_secret& b)
{
    init.init();
    return secp256k1_ec_privkey_tweak_add(a.data(), b.data()) != 0;
}

BC_API bool ec_multiply(ec_secret& a, const ec_secret& b)
{
    init.init();
    return secp256k1_ec_privkey_tweak_mul(a.data(), b.data()) != 0;
}
