...
//#if BYTE_ORDER == LITTLE_ENDIAN
    ...
    #define blk0(i) (buffer->l[i] = (rol(buffer->l[i],24)&0xFF00FF00) \
        |(rol(buffer->l[i],8)&0x00FF00FF))
#else
    # define blk0(i) block->l[i]
#endif

#define blk(i) (buffer->l[i&15] = rol(buffer->l[(i+13)&15]^buffer->l[(i+8)&15] \
...
void SHA1Transform(uint32_t state[5], const uint8_t block[SHA1_BLOCK_LENGTH])
{
    ...
    CHAR64LONG16* buffer = (CHAR64LONG16*)workspace;

    memcpy(buffer, block, SHA1_BLOCK_LENGTH);
