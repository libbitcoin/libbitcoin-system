...
void handle_resubmit(const std::error_code& ec,
    ...
    hash_digest tx_hash)
{
    log_debug() << "handle_resubmit(" << ec.message() << ", " << tx_hash << ")";
    if (ec)
        handle_confirm(ec);
}
void transaction_pool::resubmit_all()
{
    log_debug() << "transaction_pool::resubmit_all()";
    for (const transaction_entry_info& entry: pool_)
    {
        log_debug() << "  re-storing " << hash_transaction(entry.tx);
        store(entry.tx, entry.handle_confirm,
            std::bind(handle_resubmit, _1, entry.handle_confirm,
                hash_transaction(entry.tx)));
    }
    pool_.clear();
}
