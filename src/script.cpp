...
hash_digest generate_signature_hash(
...
{
    operation_stack::iterator codehash_begin;
...
bool op_checksigverify(evaluation_context& context, script_type& script,
    const transaction_type& parent_tx, uint32_t input_index)
{
    if (context.primary.size() < 2)
        return false;
...
bool op_checksig(evaluation_context& context, script_type& script,
    const transaction_type& parent_tx, uint32_t input_index)
{
...
bool op_checkmultisigverify(evaluation_context& context, script_type& script,
    const transaction_type& parent_tx, uint32_t input_index)
{
...
bool op_checkmultisig(evaluation_context& context, script_type& script,
    const transaction_type& parent_tx, uint32_t input_index)
{
...
bool run_operation(const operation& op, const transaction_type& parent_tx,
    uint32_t input_index, script_type& script, evaluation_context& context)
{
    switch (op.code)
    {
        case opcode::zero:
...
bool next_step(const transaction_type& parent_tx, uint32_t input_index,
    operation_stack::iterator it, script_type& script,
...
bool evaluate(const transaction_type& parent_tx, uint32_t input_index,
    script_type& script, evaluation_context& context)
{
    script_type script;
...
bool evaluate(script_type& input_script,
    ...
    uint32_t input_index, bool bip16_enabled)
{
    ...
    if (bip16_enabled && (output_script.type() == payment_type::script_hash))
    {
        ...
        script_type eval_script(input_context.primary.back(), true);
