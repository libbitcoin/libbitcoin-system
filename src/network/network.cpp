...
acceptor::acceptor(threadpool& pool, tcp_acceptor_ptr tcp_accept)
  : threadpool_(pool), tcp_accept_(tcp_accept)
{
}
void acceptor::accept(accept_handler handle_accept)
{
    socket_ptr socket =
        std::make_shared<tcp::socket>(threadpool_.service());
    ...
    socket_ptr socket, accept_handler handle_accept)
{
    ...
    channel_ptr channel_object =
        std::make_shared<channel>(threadpool_, socket);
    ...
    connect_handler handle_connect)
{
    resolver_ptr resolver =
        std::make_shared<tcp::resolver>(threadpool_.service());
    query_ptr query =
...
void network::listen(uint16_t port, listen_handler handle_listen)
{
    ...
    acceptor::tcp_acceptor_ptr tcp_accept =
        std::make_shared<tcp::acceptor>(threadpool_.service());
    ...
    acceptor_ptr accept =
        std::make_shared<acceptor>(threadpool_, tcp_accept);
