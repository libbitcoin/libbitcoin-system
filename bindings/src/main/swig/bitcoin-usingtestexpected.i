...
//#define quarter_hash_size (half_hash_size / 2)
//#define long_hash_size (2 * hash_size)
//#define short_hash_size 20
//#define mini_hash_size 6
...
template <size_t Size>
bool decode_base16(std::array<uint8_t, Size>& out, const std::string &in);

template <size_t Size>
std::array<uint8_t, (Size - 1) / 2> base16_literal(const char (&string)[Size]);
...
%template(HalfHash) std::array<uint8_t, 16>;
...
%template(HalfHashList) std::vector<std::array<uint8_t, 16>>;
