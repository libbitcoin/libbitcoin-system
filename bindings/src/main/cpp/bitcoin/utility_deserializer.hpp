 ...
 */
#ifndef LIBBITCOIN__UTILITY_DESERIALIZER_HPP
#define LIBBITCOIN__UTILITY_DESERIALIZER_HPP
...
utility_deserializer<Iterator, true> make_safe_deserializer(const Iterator begin,
    const Iterator end);

/// Faster deserializer (without bounds checking).
/// Intended for use with internal/protected buffers only.
template <typename Iterator>
utility_deserializer<Iterator, false> make_unsafe_deserializer(const Iterator begin);
