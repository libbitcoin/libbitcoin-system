 ...
 */
#ifndef LIBBITCOIN__MESSAGE_FILTER_LOAD_HPP
#define LIBBITCOIN__MESSAGE_FILTER_LOAD_HPP
...
class BC_API message_filter_load : public message::filter_load
{
public:
//    typedef std::shared_ptr<filter_load> ptr;
//    typedef std::shared_ptr<const filter_load> const_ptr;

    static message_filter_load factory(uint32_t version, const utility_data_chunk& data);
//    static message_filter_load factory(uint32_t version, std::istream& stream);
//    static message_filter_load factory(uint32_t version, reader& source);

    message_filter_load();
//    message_filter_load(const utility_data_chunk& filter, uint32_t hash_functions,
...
//    void set_filter(const utility_data_chunk& value);
    void set_filter(utility_data_chunk&& value);
...
//    message_filter_load& operator=(message_filter_load&& other);
    message_filter_load& assign(message_filter_load&& other);
//    void operator=(const message_filter_load&) = delete;

//    bool operator==(const message_filter_load& other) const;
    bool eq(const message_filter_load& other) const;
//    bool operator!=(const message_filter_load& other) const;
