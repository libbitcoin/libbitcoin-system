 ...
 */
#ifndef LIBBITCOIN__ENCRYPTED_KEYS_HPP
#define LIBBITCOIN__ENCRYPTED_KEYS_HPP
...
//#include <utility_byte_array.hpp>
#include <p_bool.hpp>
#include <math_ec_compressed.hpp>
#include <wallet_ek_seed.hpp>
 ...
 */
BC_API bool create_key_pair(wallet_encrypted_private& out_private,
    math_ec_compressed& out_point, const wallet_encrypted_token& token,
    const wallet_ek_seed& seed, uint8_t version, bool compressed=true);
 ...
 */
BC_API bool create_key_pair(wallet_encrypted_private& out_private,
    wallet_encrypted_public& out_public, math_ec_compressed& out_point,
    const wallet_encrypted_token& token, const wallet_ek_seed& seed, uint8_t version,
 ...
 */
BC_API bool create_token(wallet_encrypted_token& out_token,
    const std::string& passphrase, const wallet_ek_entropy& entropy);
 ...
 */
BC_API bool create_token(wallet_encrypted_token& out_token,
    const std::string& passphrase, const wallet_ek_salt& salt, uint32_t lot,
    uint32_t sequence);
 ...
 */
BC_API bool encrypt(wallet_encrypted_private& out_private, const math_ec_secret& secret,
 ...
 */
BC_API bool decrypt(math_ec_secret& out_secret, uint8_t& out_version,
    p_bool out_compressed, const wallet_encrypted_private& key,
 ...
 * @return false if the key    checksum or passphrase is not valid.
 */
BC_API bool decrypt(wallet_ec_compressed& out_point, uint8_t& out_version,
    p_bool out_compressed, const wallet_encrypted_public& key,
    const std::string& passphrase);
