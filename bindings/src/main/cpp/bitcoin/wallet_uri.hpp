 ...
 */
#ifndef LIBBITCOIN__WALLET_URI_HPP
#define LIBBITCOIN__WALLET_URI_HPP
...
//    typedef std::map<std::string, std::string> query_map;
     ...
     */
    wallet_uri_query_map decode_query() const;
    void encode_query(const wallet_uri_query_map& map);

    wallet::uri getValue() {
		return value;
	}

	void setValue(wallet::uri value) {
		this->value = value;
	}
private:
	wallet::uri value;
