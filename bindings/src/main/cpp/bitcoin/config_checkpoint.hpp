 ...
 */
#ifndef LIBBITCOIN__CONFIG_CHECKPOINT_HPP
#define LIBBITCOIN__CONFIG_CHECKPOINT_HPP
 ...
 */
class BC_API config_checkpoint
{
public:
    /**
...
//    typedef std::vector<checkpoint> list;
     ...
     */
    static config_checkpoint_list sort(const config_checkpoint_list& checks);
     ...
     * @param[in]  checks  The list of checkpoints.
     */
    static bool covered(size_t height, const config_checkpoint_list& checks);
     ...
     */
    config_checkpoint(const config_checkpoint& other);
     ...
     */
    config_checkpoint(const std::string& value);
     ...
     */
    config_checkpoint(const std::string& hash, size_t height);
...
//    bool operator==(const checkpoint& other) const;
    bool eq(const config_checkpoint& other) const;
...
//        const checkpoint& argument);
	...
	config::checkpoint getValue() {
		return value;
	}

	void setValue(config::checkpoint value) {
		this->value = value;
	}
private:
