 ...
 */
#ifndef LIBBITCOIN__FORMATS_BASE_16_HPP
#define LIBBITCOIN__FORMATS_BASE_16_HPP
 ...
 */
BC_API std::string encode_base16(utility_data_slice data);

/**
 * Convert a hex string into bytes.
 * @return false if the input is malformed.
 */
BC_API bool decode_base16(utility_data_chunk& out, const std::string &in);
 ...
 */
BC_API std::string encode_hash(math_hash_digest hash);
 ...
 * @return false if the input is malformed.
 */
BC_API bool decode_hash(math_hash_digest& out, const std::string& in);

/**
 * Convert a hex string literal into a bitcoin_hash.
 * The bitcoin_hash format is like base16, but with the bytes reversed.
 */
//BC_API math_hash_digest hash_literal(const char (&string)[2 * libbitcoin::hash_size + 1]);
BC_API math_hash_digest hash_literal(const std::string& string);
