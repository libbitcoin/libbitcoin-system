 ...
 */
#ifndef LIBBITCOIN_SYSTEM_INTRINSICS_INTEL_128_HPP
#define LIBBITCOIN_SYSTEM_INTRINSICS_INTEL_128_HPP
...
{
    return xor_(a, _mm_set1_epi64x(-1));
}
...
{
    if constexpr (S == bits<uint8_t>)
        return add<S>(a, _mm_set1_epi8(K));
    if constexpr (S == bits<uint16_t>)
        return add<S>(a, _mm_set1_epi16(K));
    if constexpr (S == bits<uint32_t>)
        return add<S>(a, _mm_set1_epi32(K));
    if constexpr (S == bits<uint64_t>)
...
INLINE xint128_t byteswap(xint128_t a) NOEXCEPT
{
    return _mm_shuffle_epi8(a, set<xint128_t>(
