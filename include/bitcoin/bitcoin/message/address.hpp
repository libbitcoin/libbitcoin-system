...
public:
    ...
    static address factory_from_data(const uint32_t version, const data_chunk& data);
    static address factory_from_data(const uint32_t version, std::istream& stream);
    static address factory_from_data(const uint32_t version, reader& source);
