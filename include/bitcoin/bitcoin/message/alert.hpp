...
public:
    ...
    static alert factory_from_data(const uint32_t version, const data_chunk& data);
    static alert factory_from_data(const uint32_t version, std::istream& stream);
    static alert factory_from_data(const uint32_t version, reader& source);
