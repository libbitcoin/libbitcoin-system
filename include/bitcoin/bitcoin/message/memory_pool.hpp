...
public:
    ...
    static memory_pool factory_from_data(const uint32_t version, const data_chunk& data);
    static memory_pool factory_from_data(const uint32_t version, std::istream& stream);
    static memory_pool factory_from_data(const uint32_t version, reader& source);
